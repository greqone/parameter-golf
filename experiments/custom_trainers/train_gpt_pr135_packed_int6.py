...
INT6_PACK_ENABLED = bool(int(os.environ.get("INT6_PACK_ENABLED", "1")))
...
def pack_int6_tensor(q: Tensor) -> Tensor:
    flat = q.detach().to(torch.int16).reshape(-1) + 32
    pad = (-flat.numel()) % 4
    if pad:
        flat = torch.cat([flat, torch.zeros(pad, dtype=torch.int16)], dim=0)
    groups = flat.reshape(-1, 4).to(torch.int32)
    c0, c1, c2, c3 = groups.unbind(dim=1)
    b0 = c0 | ((c1 & 0x03) << 6)
    b1 = ((c1 >> 2) & 0x0F) | ((c2 & 0x0F) << 4)
    b2 = ((c2 >> 4) & 0x03) | (c3 << 2)
    return torch.stack((b0, b1, b2), dim=1).to(torch.uint8).reshape(-1).contiguous()


def unpack_int6_tensor(packed: Tensor, shape: torch.Size) -> Tensor:
    total = math.prod(shape)
    bytes3 = packed.detach().to(torch.int32).reshape(-1, 3)
    b0, b1, b2 = bytes3.unbind(dim=1)
    c0 = b0 & 0x3F
    c1 = ((b0 >> 6) & 0x03) | ((b1 & 0x0F) << 2)
    c2 = ((b1 >> 4) & 0x0F) | ((b2 & 0x03) << 4)
    c3 = (b2 >> 2) & 0x3F
    flat = torch.stack((c0, c1, c2, c3), dim=1).reshape(-1)[:total]
    return (flat.to(torch.int16) - 32).to(torch.int8).reshape(shape).contiguous()
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            result[name + ".q"] = pack_int6_tensor(q) if INT6_PACK_ENABLED else q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6_packed" if INT6_PACK_ENABLED else "int6"}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
