...
class Hyperparameters:
    ...
    mlp_gate_groups = int(os.environ.get("MLP_GATE_GROUPS", 0))
    mlp_gate_bias_init = float(os.environ.get("MLP_GATE_BIAS_INIT", 0.0))
    attn_every = int(os.environ.get("ATTN_EVERY", "1"))
    mixer_kernel = int(os.environ.get("MIXER_KERNEL", "4"))
    pit_diag = bool(int(os.environ.get("PIT_DIAG", "0")))
...
class CausalDepthwiseConvMixer(nn.Module):
    def __init__(self, dim: int, kernel_size: int):
        super().__init__()
        if kernel_size <= 0:
            raise ValueError(f"MIXER_KERNEL must be positive, got {kernel_size}")
    ...
    def forward(self, x: Tensor) -> Tensor:
        seqlen = x.size(1)
        y = self.conv(x.transpose(1, 2))[:, :, :seqlen].transpose(1, 2).contiguous()
...
class MLP(nn.Module):
    def __init__(
        ...
        if gate_groups > 0:
            if hidden % gate_groups != 0:
                raise ValueError(f"MLP hidden size {hidden} must be divisible by MLP_GATE_GROUPS={gate_groups}")
            self.gate = CastedLinear(dim, gate_groups, bias=True)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.gate is not None:
            gate = 2.0 * torch.sigmoid(self.gate(gate_input))
            gate = gate.repeat_interleave(x.size(-1) // self.gate_groups, dim=-1).to(dtype=x.dtype)
            x = x * gate
...
class Block(nn.Module):
    def __init__(
        ...
        qk_gain_init: float,
        mlp_hidden: int = 0,
        mlp_gate_groups: int = 0,
        mlp_gate_bias_init: float = 0.0,
        ...
        self.attn = (
            ...
            if use_attention
            else CausalDepthwiseConvMixer(dim, mixer_kernel)
        )
        self.mlp = MLP(dim, mlp_mult, mlp_hidden, mlp_gate_groups, mlp_gate_bias_init)
...
class GPT(nn.Module):
    def __init__(
        ...
        qk_gain_init: float,
        mlp_gate_groups: int,
        mlp_gate_bias_init: float,
        attn_every: int,
        mixer_kernel: int,
        pit_diag: bool,
        pit_log_scale_init: float,
    ):
        super().__init__()
        ...
        self.pit_log_scale = (
            nn.Parameter(torch.full((model_dim,), pit_log_scale_init, dtype=torch.float32))
            if pit_diag
            else None
        )
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    mlp_gate_groups=mlp_gate_groups,
                    mlp_gate_bias_init=mlp_gate_bias_init,
                    use_attention=(i % attn_every == 0),
                    mixer_kernel=mixer_kernel,
                )
    ...
    def pit_scale(self, dtype: torch.dtype) -> Tensor | None:
        if self.pit_log_scale is None:
            return None
        return torch.exp(self.pit_log_scale.clamp(-4.0, 4.0)).to(dtype=dtype)

    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        x = self.tok_emb(input_ids)
        pit_scale = self.pit_scale(x.dtype)
        if pit_scale is not None:
            x = x * pit_scale.reciprocal()[None, None, :]
...
def main() -> None:
    ...
    zeropower_via_newtonschulz5 = maybe_compile(
        zeropower_via_newtonschulz5, enabled=args.enable_torch_compile
    )
    ...
    cudnn_enabled, flash_enabled, mem_efficient_enabled, math_enabled = configure_sdp_backend(args.sdp_backend)
    ...
    base_model = GPT(
        ...
        mlp_gate_groups=args.mlp_gate_groups,
        mlp_gate_bias_init=args.mlp_gate_bias_init,
        attn_every=args.attn_every,
        mixer_kernel=args.mixer_kernel,
        pit_diag=args.pit_diag,
    ...
    log0(f"mlp_gate_groups:{args.mlp_gate_groups} mlp_gate_bias_init:{args.mlp_gate_bias_init}")
    log0(f"attn_every:{args.attn_every} mixer_kernel:{args.mixer_kernel}")
    log0(f"pit_diag:{args.pit_diag} pit_log_scale_init:{args.pit_log_scale_init}")
    log0(f"torch_compile:{args.enable_torch_compile} sdp_backend:{args.sdp_backend}")
